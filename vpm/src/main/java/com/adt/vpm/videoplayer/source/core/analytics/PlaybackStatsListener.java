...
public final class PlaybackStatsListener
  ...
  private static final class PlaybackStatsTracker {
    ...
    public void onFatalError(EventTime eventTime, Exception error) {
      fatalErrorCount++;
      if (keepHistory) {
        fatalErrorHistory.add(new PlaybackStats.EventTimeAndException(eventTime, error));
      }
      hasFatalError = true;
      isInterruptedByAd = false;
      isSeeking = false;
    ...
    public void onNonFatalError(EventTime eventTime, Exception error) {
      nonFatalErrorCount++;
      if (keepHistory) {
        nonFatalErrorHistory.add(new PlaybackStats.EventTimeAndException(eventTime, error));
      }
    }
    ...
    private void maybeUpdatePlaybackState(EventTime eventTime, boolean belongsToPlayback) {
      @PlaybackStats.PlaybackState int newPlaybackState = resolveNewPlaybackState();
    ...
    private @PlaybackStats.PlaybackState int resolveNewPlaybackState() {
      if (isFinished) {

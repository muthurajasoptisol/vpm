...
import com.adt.vpm.videoplayer.source.common.util.ParsableNalUnitBitArray;
...
/*package*/ final class RtpH264PayloadReader implements RtpPayloadReader {
    ...
    private int getNalUnitType(ParsableByteArray packet, int offset) {
        return packet.getData()[offset] & 0x1F;
    }
    ...
    private void handleAggregationNalUnit(ParsableByteArray packet) {
        ...
        while (offset < limit) {
            ...
            packet.readBytes(nalLength.getData(), 0, 2);
            ...
            if (hasOutputFormat) {
                ...
                byte[] data = Arrays.copyOfRange(packet.getData(), offset + 2, offset + 2 + nalUnitLength);
    ...
    private void handleFragmentationNalUnit(ParsableByteArray packet) {
        int limit = packet.bytesLeft();
        int indicatorFU = packet.getData()[0] & 0xFF;
        int headerFU = packet.getData()[1] & 0xFF;
        ...
        if (isFirstFragmentUnit) {
            ...
            byte[] fragmentUnit = Arrays.copyOfRange(packet.getData(), 1, limit);
            ...
            byte[] fragmentUnit = Arrays.copyOfRange(packet.getData(), 2, limit);
            fragments.appendFragmentUnit(fragmentUnit, 0, fragmentUnit.length);
            ...
            switch (nalUnitType) {
                ...
                case NAL_UNIT_TYPE_SPS:
                    if (!hasOutputFormat || detectAccessUnits) {
                        NalUnitUtil.SpsData spsData = NalUnitUtil.parseSpsNalUnit(nalUnit.getData(), 0, nalUnit.limit());
                        sps.append(spsData.seqParameterSetId, spsData);

                        if (spsNalUnit == null) {
                            spsNalUnit = new ParsableByteArray(Arrays.copyOf(nalUnit.getData(), nalUnit.limit()));
                        }
                    }
                    break;
                ...
                case NAL_UNIT_TYPE_PPS:
                    if (!hasOutputFormat || detectAccessUnits) {
                        NalUnitUtil.PpsData ppsData = NalUnitUtil.parsePpsNalUnit(nalUnit.getData(), 0, nalUnit.limit());
                        pps.append(ppsData.picParameterSetId, ppsData);

                        if (ppsNalUnit == null) {
                            ppsNalUnit = new ParsableByteArray(Arrays.copyOf(nalUnit.getData(), nalUnit.limit()));
                        }
                    }
                    break;
                ...
                default:
                    if ((allowNonIdrKeyframes && nalUnitType == NAL_UNIT_TYPE_NON_IDR)
                        ...
                        sliceHeader.parseNalUnit(nalUnitType, nalUnit.getData(), nalUnit.limit());
                    }
            }
            ...
            if (!hasOutputFormat) {
                if (spsNalUnit != null && ppsNalUnit != null) {
                    List<byte[]> initializationData = CodecSpecificDataUtil.buildH264SpecificConfig(
                        spsNalUnit.getData(), ppsNalUnit.getData());

                    NalUnitUtil.SpsData spsData = NalUnitUtil.parseSpsNalUnit(
                            spsNalUnit.getData(), 0, spsNalUnit.limit());

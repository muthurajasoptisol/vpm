...
import com.adt.vpm.videoplayer.source.rtp.format.FormatSpecificParameters;
...
/*package*/ final class RtpH265PayloadReader implements RtpPayloadReader {
    ...
    private int getNalUnitType(ParsableByteArray packet, int offset) {
        return (packet.getData()[offset] >> 1) & 0x3F;
    }
    ...
    private void handleAggregationNalUnit(ParsableByteArray packet) {
        ...
        while (offset < limit) {
            ...
            if (hasOutputFormat) {
                ...
                byte[] data = Arrays.copyOfRange(packet.getData(), offset + 2, offset + 2 +
                    nalUnitLength);
    ...
    private void handleFragmentationNalUnit(ParsableByteArray packet) {
        int limit = packet.bytesLeft();
        int headerFU = packet.getData()[2] & 0xFF;
        int nuhTemporalIdPlus1 = packet.getData()[1] & 0x03;
         ...
         */
        int nalUnitType = packet.getData()[2] & 0x3F;
        ...
        if (isFirstFragmentUnit) {
            ...
            byte[] fragmentUnit = Arrays.copyOfRange(packet.getData(), 3, limit);
            fragments.appendFragmentUnit(fragmentUnit, 0, fragmentUnit.length);
        ...
        private void consume(int nalUnitType, ParsableByteArray nalUnit) {
            ...
            switch (nalUnitType) {
                ...
                case NAL_UNIT_TYPE_VPS_NUT:
                    if (!hasOutputFormat) {
                        if (vps == null) {
                            vps = Arrays.copyOf(nalUnit.getData(), nalUnit.limit());
                        }
                    }
                    break;
                ...
                case NAL_UNIT_TYPE_SPS_NUT:
                    if (!hasOutputFormat) {
                        if (sps == null) {
                            sps = Arrays.copyOf(nalUnit.getData(), nalUnit.limit());
                        }
                    }
                    break;
                ...
                case NAL_UNIT_TYPE_PPS_NUT:
                    if (!hasOutputFormat) {
                        if (pps == null) {
                            pps = Arrays.copyOf(nalUnit.getData(), nalUnit.limit());
                        }
                    }
                    break;
            }

...
import static com.adt.vpm.videoplayer.source.common.util.Assertions.checkArgument;
  ...
  private synchronized int readSampleMetadata(
    ...
    while ((hasNextSample = hasNextSample())) {
      ...
      if (timeUs < decodeOnlyUntilUs
              && MimeTypes.allSamplesAreSyncSamples(formats[relativeReadIndex].sampleMimeType, formats[relativeReadIndex].codecs)) {
        readPosition++;
      } else {
        break;
      }
